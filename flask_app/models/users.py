...
class User:
    ...
    def save(cls, formulario):
        ...
        result = connectToMySQL('agro').query_db(query, formulario) #me regresan el nuevo ID de la persona registrada
        #result = 5
        return result
    ...
    def valida_usuario(formulario):
        ...
        if len(formulario['first_name']) < 3 :
            flash('Nombre debe de tener al menos 3 caracteres', 'registro1')
            es_valido = False
        
        if len(formulario['last_name']) < 3:
            flash('Apellido debe de tener al menos 3 caracteres', 'registro1')
            es_valido = False

        #Verificar que el email tenga formato correcto - EXPRESIONES REGULARES
        if not EMAIL_REGEX.match(formulario['email']):
            flash('E-mail inválido', 'registro1')
            es_valido = False
        
        #Password con al menos 6 caracteres
        if len(formulario['password']) < 6:
            flash('Contraseña debe tener al menos 6 caracteres', 'registro1')
            es_valido = False
        
        #Verificamos que las contraseñas coincidan
        if formulario['password'] != formulario['confirm_password']:
            flash('Contraseñas no coinciden', 'registro1')
            es_valido = False
        ...
        if len(results) >= 1:
            flash('E-mail registrado previamente', 'registro1')
            es_valido = False
    ...
    def get_by_email(cls, formulario):
        ...
        #}
        query = "SELECT * FROM users WHERE email = %(email)s"
        result = connectToMySQL('agro').query_db(query, formulario) #Los SELECT regresan una lista
    ...
    def get_by_id(cls, formulario):
        ...
        result = connectToMySQL('agro').query_db(query, formulario) #RECIBIMOS UNA LISTA
